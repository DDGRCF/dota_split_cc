...
template <typename T> inline int sig(T d) { return (d > kEps) - (d < -kEps); }
...
template <typename T> inline T area(Point<T> *q, const uint8_t &m) {
  q[m] = q[0];
  T res{0.};
  for (uint8_t i = 0; i < m; ++i) {
...
template <typename T> inline void point_swap(Point<T> *a, Point<T> *b) {
  Point<T> tmp = *a;
  *a = *b;
  *b = tmp;
}
...
inline T rotated_boxes_intersection(Point<T> (&pts1)[kMaxn], const uint8_t &n1,
                                    ...
                                    const uint8_t &n2) {
  if (area<T>(pts1, n1) < 0)
    point_reverse<T>(pts1, pts1 + n1);
  if (area<T>(pts2, n2) < 0)
    point_reverse<T>(pts2, pts2 + n2);
  pts1[n1] = pts1[0];
  pts2[n2] = pts2[0];
  T res{0.};
...
inline T single_poly_iou_rotated(T const *const box1_raw,
  ...
  static_assert(is_same<typename decay<T>::type, float>::value ||
                    ...
                    is_same<typename decay<T>::type, long double>::value,
